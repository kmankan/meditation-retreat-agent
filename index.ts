...
const inputs = [
  ...
  "https://kynanmeditation.net/",
  "https://www.jhourney.io/",
  "https://kellyboys.org/",
  "https://www.imsb.org/events/category/retreat/list/",
  "https://www.spiritrock.org/programs/retreats"
]

const getRetreatInformation = (url:string) => {
  function getWebsiteName(url: string): string {
    try {
      const parsedUrl = new URL(url);
      const hostnameParts = parsedUrl.hostname.split('.');
      return hostnameParts[hostnameParts.length - 2] || 'default';
    } catch (error) {
      return 'default';
    }
  }
